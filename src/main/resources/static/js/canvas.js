...
const paintToggle = () => {
  ...
  for (let row = 0; row < TOGGLE_ROWS; row++) {
    for (let col = 0; col < TOGGLE_COLS; col++) {
      ...
      ctx.fillStyle = 'black';
      if(col == 0){
        ctx.fillText(text, ((col + 1) * TILE_WIDTH - TILE_WIDTH / 2) - 20, (((row + 1) * TILE_HEIGHT) - TILE_HEIGHT / 2));
      } else {
        ctx.fillText(text, ((col + 1) * TILE_WIDTH) - TILE_WIDTH / 2, ((row + 1) * TILE_HEIGHT) - TILE_HEIGHT / 2);
      }
    }
  }
};
...
const paintOnClick = event => {
  ...
  if (col == 1) {
    ...
    ctx.fillStyle = "black"
    ctx.fillText("on", ((col + 1) * TILE_WIDTH) - TILE_WIDTH / 2, ((row + 1) * TILE_HEIGHT) - TILE_HEIGHT / 2);
    ...
    ctx.fillStyle = 'black';
    ctx.fillText("off", ((col + 2) * TILE_WIDTH) - TILE_WIDTH / 2, ((row + 1) * TILE_HEIGHT) - TILE_HEIGHT / 2);
    makeGrid();
    ...
    ctx.fillRect(TILE_WIDTH * col, TILE_HEIGHT * row, TILE_WIDTH, TILE_HEIGHT)
    ctx.fillStyle = "black"
    ctx.fillText("off", ((col + 1) * TILE_WIDTH) - TILE_WIDTH / 2, ((row + 1) * TILE_HEIGHT) - TILE_HEIGHT / 2);
    ...
    ctx.fillRect(TILE_WIDTH * (col - 1), TILE_HEIGHT * row, TILE_WIDTH, TILE_HEIGHT)
    ctx.fillStyle = "black"
    ctx.fillText("on", ((col) * TILE_WIDTH) - TILE_WIDTH / 2, ((row + 1) * TILE_HEIGHT) - TILE_HEIGHT / 2);
    makeGrid();
    // TODO: using the selected toggle setting that you defined in a previous
    ...
    // 'selection' array, so be sure to check for that first!
    if (selection.includes(toggleSetting)) {
      selection.splice(selection.indexOf(toggleSetting),1);
    }
  }
